...
crownFeel <- function(fileFinder, cutWindow = c(-1000, -1000, 2000), ipad = FALSE, doReferencedOnly = FALSE, referenced = FALSE,
  ...
  if(mode == "COMP"){
    ...
    if(mode == "ALLGO"){
      ...
      if(updateTreeList){
        ...
        write.table(clustList, file = clustList.name,
                    row.names = FALSE, sep = "\t")
      }
      ...
      if(useTreeFile != ""){
        if(file.exists(paste0(dbhPath, "seedLAS_cylinders.las"))){
          ...
          file.rename(from = paste0(dbhPath, "seedLAS_cylinders.las"),
                      to = oldFile)
        }
      }
      if(file.exists(paste0(dbhPath, "seedLAS_cylinders.las"))){
        ...
        if(tileClipping == 0){
          cat("Efficiency improvement: Cutting slice from", min(clustList$z, na.rm = TRUE), "to",
              max(clustList$z, na.rm = TRUE), "m.\n")
          cat("Reducing total", thMk(totalCloud@header@PHB$`Number of point records`), "points to ")
          reducedTotalCloud <- filter_poi(totalCloud, Z > min(clustList$z, na.rm = TRUE) - frame.down,
        ...
        if(tileClipping > 0){
          ...
          tryCatch(
            {
              ...
              text(x = clustList$x + 0.5 + cexFactor,
          ...
          for(i in 1:length(clustList[, 1])){
            ...
            clustCloud <- clip_circle(reducedTotalCloud, xcenter = nowClust$x, ycenter = nowClust$y,
      ...
      if (is.element("R", colnames(stemCloud@data))) {
        ...
        removes <- c(which(colnames(stemCloud@data) == "R"),
  ...
  cat("We remain starting with", thMk(seeds@header@PHB$`Number of point records`), "seed points (=",
  ...
  if(voxelSize != 0){
    ...
    startRow <- c(rep(round(voxelSize / 100, 2) + 0.0001, start.times - last.quart),
                  rep(round(voxelSize / 100 * 1.6, 2) + 0.0001, last.quart)) # times sqrt(2) for circleish shape on the ground
  ...
  cat("Adding the fast climbing", reachableMaxHeight,
  ...
  #### Crowns: loops ####
  prgsFrame <- data.frame("round"=0L, "dist"=0, "cumDist"=0, "maxh"=0, "pctAdded"=0, "treesGrow"=0,
  ...
  for(j in 1:totalRuns){
    ...
    if(j == 1){
      ...
      cat(fileFinder, ": Iteration ", sprintf("%03d", j),
          " on _", format(Sys.time(), "%a%d. %X"), "_", sep="")
   ...
   #         k = howManyClosestPoints, searchtype = "standard")
    ...
    cat("Assigning", thMk(assignedPoints), "pts of total",
        thMk(blankSetLength),
        "(nowShare =", nowShare,"%)\n")
    ...
    cat("Elapsed time: NB", round(timeNB, 1), units(timeNB),
  ...
  if(voxelSize != 0){
    ...
    cat(thMk(totalPoints), "pts in allocationCloud (blanks) and",
    ...
    if(totalPoints > 500000000){
    ...
    } else
      {
        ...
        cat("Assigning", thMk(assignedPoints),
