...
grabDBH_i <- function(seedLAS, nowMeta, 
                      filterDIST = 50, filterINT = 50, filterKDE = 94, 
                      circleImagePath = paste0(getwd(), "/circleFits/")){
  
  
  if(!dir.exists(circleImagePath)) dir.create(circleImagePath, recursive = T)
  dbhStart <- Sys.time()
  
  nowId <- nowMeta$id
  nowDBHref <- nowMeta$dbh.ref
  ...
  nowMeta$nP.orig <- nPointsOri
  
  if(nPointsOri < 10){
    cat("\n",nowMeta$id, "- too few points in seed, skip this one\n")
    return(nowMeta)
  }
  
  if(filterDIST < 100){
    # DISTANCE FILTERING
    thresholdDIST <- quantile(clustCloud@data$SensorDistance, filterDIST / 100) # all above are 5 %
  ...
  par(mfrow = c(1, 1))
  {
    png(filename = paste0(circleImagePath, nowId, "_circles.png"), width = 1400, height = 800, type = "cairo")
    ...
    for(j in 1:length(slices)){
      ...
      class(try({
        ...
        points(nowMeta$x.ref, nowMeta$y.ref, col = "grey", cex = 2, pch = 4)
        ...
        if(!is.null(find.clust.circ)){
          ...
          mtext(paste0(" move ", round(100*sqrt((x.circle - nowMeta$x.ref)^2 + (y.circle - nowMeta$y.ref)^2), 1)),
          ...
          lines(x = c(x.circle, nowMeta$x.ref),
                y = c(y.circle, nowMeta$y.ref), col = "grey")
    ...
    if(!is.null(circles)){
      ...
      nowMeta$dbh <- dbh.mean
      nowMeta$dbh.gam <- round(dbh.gam.mean,2)
      nowMeta$sd.gam <- gam.sd.mean
      nowMeta$var.dbh <- round(sd(circles[3, ]*200, na.rm = T),2)
      nowMeta$var.dbh.gam <- round(sd(circles[6, ], na.rm = T),2)
      nowMeta$var.sd.gam <- round(sd(circles[7, ], na.rm = T),4)
      # HUOM: Not real variance, but sd just to prevent name confusion with gam sd (scattering)
      nowMeta$x <- x.mean
      nowMeta$y <- y.mean
      nowMeta$slices <- length(slices)
      nowMeta$move <- round(100*sqrt((x.mean - nowMeta$x.ref)^2 + (y.mean - nowMeta$y.ref)^2), 1)
    } else {
      #cat("No DBH estimated in", round(timeNB, 1), units(timeNB), "\n")
    }
    
    timeNB <- as.numeric(difftime(dbhStop, dbhStart, units = "secs"))
    nowMeta$timeSec <- round(timeNB, 1)
...
grabDBH <- function(fileFinder, 
  ...
  circleImagePath <- paste0(dbhPath, "circleFits/")
  if(!dir.exists(circleImagePath)) dir.create(circleImagePath)
